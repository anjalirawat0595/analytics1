...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5)) # minimum products purchases 2 and max. 5
...
subset2 = subset(rules2, subset=lhs %ain% c('baking powder','soda') ) # and condition
inspect(subset2) # all items  %ain%
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') ) # or condition
