...
head(inputData) # for what gre/gpa predicting chances of being selected
dim(inputData) # details of rows and columns for display purpose
inputData
summary(inputData)
#sd of all columns 
sd(inputData$admit)
inputData[,1]
for (i in 1:4) { print(sd(inputData[,i]))} #(all rows in 4 column , i-> for all rows )
#use apply functions
sapply(inputData, sd) # simple apply
lapply(inputData, sd) # list form apply
...
data$rank = factor(data$rank) #convert it to factor
...
(t1= xtabs(~ admit + rank, data = data)) #xtabs for 
addmargins(t1) #column wise/row wise summary
prop.table(t1,2) # prop. -> showing in percentage
?prop.table
#xtabs(~ gear + cyl + am , data=mtcars)
#create Logistic Model
mylogit = glm(admit ~ gre + gpa + rank, data = data, family = "binomial") # created a logistic model to predict just like linear regression
...
## odds ratios only
exp(coef(mylogit)) i#intercept coeficient -> logarithms and exponentials
library(dplyr)
