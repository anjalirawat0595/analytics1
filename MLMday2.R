...
omni = omni1
head(omni) # heading wise data display
str(omni)
nrow(omni) # no. of rows
dim(omni) # rows and col
#MLR  Create Multiple Linear Regression
# we want to see how Sales Qty depend on Price and Promotion Values
fit2 = lm(sales ~ price + promotion, data=omni) # sales dependent on the price and promotion
...
fitted(fit2) # predicted values for input/actual
omni$sales # actual sales
residuals(fit2) #difference b/w predicted and actual sales
summary(residuals(fit2)) #min., 1st quantile, median, mean, max, 3rd quantile
summary(fit2)
#Predict SalesQty for new combination of Values----

#create a dataframe of new sample values
range(omni$price) ; range(omni$promotion) # range min.-max.
(ndata2 = data.frame(price=c(60,80), promotion=c(300,400)))
p2sales = predict(fit2, newdata=ndata2)
cbind(ndata2, p2sales)
head(omni) # hading-wise presentation of data
#Assumptions
par(mfrow=c(2,2)) # multiple frames for row (2R 2C)
plot(fit2)
