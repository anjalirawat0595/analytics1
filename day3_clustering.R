...
head(iris)
dim(iris)
# K means clustering signifies no. of groups
data = iris[, -5] 
...
set.seed(123)
fit =  kmeans(normalized_data, centers=3) # defining clusters
...
for (i in 1:5) Cluster_Variability[i] <- kmeans(normalized_data,centers=i, nstart=4)$tot.withinss # its a loop by 'for' 1:5 we are giving values by defining i, nstart= try 4 set of clusters and find the best
